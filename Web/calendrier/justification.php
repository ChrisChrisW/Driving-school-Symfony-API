...
<script>
    ...
    async function getData() {
        let url = 'http://localhost:8000/api/courseDates?isConfirm=1&isAchieve=1'
                ...
                data.map((item) => {
                    ...
                    if(coursesCell) {
                        coursesCell.innerHTML = dayDate + "<br><br><b>" +
                            ...
                            '<br><i class="fa fa-window-close" style="cursor: pointer" id="delete-item" data-item='+ item.id + '></i>'
                        ;
                    }
                })
            })
        ...
        if(items) {
            items.forEach(item => {
                item.addEventListener("click", () => {
                    if (confirm("êtes vous sûr de supprimer le cours ?")) {
                        fetch('http://localhost:8000/api/courseDate/absenceJustification/' + item.getAttribute("data-item"), {
                            method: 'PATCH',
                            ...
                            body: JSON.stringify({ "absenceJustification" : true })
                        })
                            .then(response => response.json())
                            .then(data => {
                                console.log(data)
